...
#  i guess we assume the file is done being read at his point, so
FILE_DATA_COMPLETE = True
...
while running:
    ...
    if FILE_DATA_COMPLETE:
        # --- your existing bouncing / highlighting logic here ---
        pass
    else:
        # Just idle until file data is ready
        pygame.time.wait(50)   # that should be long enough
    ...
    while True:
        ...
        if current_index in beat_chord_word_data and rising:
            ...
            if BEAT_NUMBERS:  # avoid ZeroDivisionError
                current_beat_number = BEAT_NUMBERS[bounce_count % len(BEAT_NUMBERS)]

                # Prevent repeated triggers for the same beat
                if current_beat_number != last_beat_number or current_index != last_index:
                   last_beat_number = current_beat_number
                   last_index = current_index
                   if MEASURING:
                       print("[X] Beat", current_beat_number, "→", current_beat_data,
                             "at", round(pygame.time.get_ticks()/1000, 2), "s",
                ...
                for beat_num, chord, word in current_beat_data:
                    if beat_num == current_beat_number:
                        ...
                        if DEBUG_BEAT_ANALYSIS:
                            print(
                                f"Beat {beat_num}: chord='{chord}' word='{word}' "
                                f"| Ball={should_highlight_ball} "
                                f"Chord={should_highlight_chord} "
                                f"Word={should_highlight_word}"
                            )
                        break
...
#                    breakpoint()
        ...
        if dt <= BOUNCE_DURATION:
            ...
            if MEASURING == True:
                elapsed_time = pygame.time.get_ticks() / 1000.0  # seconds
                print("[Y] Bounce", bounce_count, "at", round(elapsed_time, 2), "s")
            ...
            if bounce_count >= BOUNCES_PER_LINE:
                ...
                if MEASURING:
                    elapsed_time = pygame.time.get_ticks() / 1000.0  # seconds
                    print("[Z] New line", song_line_number, "current_index", current_index,"at", round(elapsed_time, 2), "s")
