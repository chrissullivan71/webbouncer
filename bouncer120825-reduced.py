...
LYRIC_FILE = r"assets\dont think twice.txt"  # r"assets\lyrick_beats.txt"
...
try:
    with open(LYRIC_FILE, "r", encoding="utf-8") as f:
        song_data = [line.rstrip() for line in f if line.strip()]
except FileNotFoundError:
    if DEBUG_FILE_PARSING:    
        print(f"Error: Could not find {LYRIC_FILE}.")
    song_data = ["[Lyrics file missing]"]
...
BOUNCE_DURATION = 0.625 # 0.898438  # seconds per bounce
...
while running:
    ...
    while True:
        ...
        if current_index in beat_chord_word_data:
            ...
            current_beat_number = bounce_count + 1
                        ...
                        if beat_num == current_beat_number:
                            ...
                            for b_num, char_index in beat_positions:
                                if b_num == beat_num:
                                    ...
                                    if word:
                                        skip_lyric_positions.append((char_index, word))
                                    break
                            break
